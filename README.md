...
- Summary comment: สรุป scores + recommendation บน PR thread
- Inline comments: comment บนบรรทัดที่เจอ HIGH/MEDIUM findings
- Fix suggestions แนบทุก comment
...
| Method | Path | คำอธิบาย |
|--------|------|----------|
| `GET` | `/api/settings/auto-comment` | ดูสถานะ auto-comment (enabled: true/false) |
| `POST` | `/api/settings/auto-comment/enable` | เปิด auto-comment |
| `POST` | `/api/settings/auto-comment/disable` | ปิด auto-comment |
...
- **Auto-Comment toggle:** เปิด/ปิดการส่ง comment กลับไปบน Azure DevOps PR อัตโนมัติ
...
ระบบจะส่ง comment กลับไปบน Azure DevOps PR อัตโนมัติหลัง review เสร็จ (เมื่อเปิดใช้งาน)

### Comment Types

**1. Summary Comment** (บน PR thread)
- สรุป recommendation (Approve / Request Changes / Comment)
- จำนวน findings แยกตาม severity (HIGH/MEDIUM/LOW)
- Score table (Logic, Security, Tests, Style, Architecture)

**2. Inline Comments** (บนบรรทัดที่เจอบั๊ก)
- เฉพาะ HIGH และ MEDIUM findings เท่านั้น
- แสดง OWASP tag
- Code snippet ที่มีปัญหา
- Fix suggestion

### เปิด/ปิด

**ผ่านหน้าเว็บ:** ไปที่ Settings → Auto-Comment → toggle

**ผ่าน API:**

```bash
# เปิด
curl -X POST http://localhost:9100/api/settings/auto-comment/enable

# ปิด
curl -X POST http://localhost:9100/api/settings/auto-comment/disable

# ดูสถานะ
curl http://localhost:9100/api/settings/auto-comment
```

### PAT Scope ที่ต้องการ

Auto-comment ต้องการ PAT scope: **Code (Read + Write)**
